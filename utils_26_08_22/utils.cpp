...
QString chars1251_2str(char* ch)
{
    ...
    str=defaultTextCodec->toUnicode(ch);
    return str;
}
//--------------------------------------------------------
...
{
    QTextCodec* defaultTextCodec = QTextCodec::codecForName("Windows-1251");
    QByteArray qb = defaultTextCodec->fromUnicode(s);
    strcpy(dst,qb.data());
}
//--------------------------------------------------------
...
{
    QString str;
    QStringList str_list=str_number.split(separator);
...
uint get_rand(uint min,uint max)
{
    ...
    static int v=((CURR_TIME_IN_MS)&0xffff)+rand();
    v++;
    srand(v);

    uint delta=max-min+1;

    if((min<=RAND_MAX)&&(max<=RAND_MAX))
    {
         retval=rand()%delta+min;
    }
    else if(min>=RAND_MAX)
    {
        uint shift=get_rand(0,16);
        if(max>min*2)
        {
            retval=((rand()%max)+min)<<shift;
            retval|=(rand()%max);
        }
        else
        {
        ...
        if(retval>max)
            retval=retval%delta+min;

    }
    else if((min<RAND_MAX)&&(max>RAND_MAX))
    {
        uint v1=get_rand(min,RAND_MAX);
        uint v2=get_rand(RAND_MAX,max);
        int f=rand();
        retval=f&1?v1:v2;
    }
    else
...
QString add_left_zeros(quint64 num, quint64 symbols_num)
{
    ...
    if(digs_in_num<symbols_num)
    {
        int zeros_num=symbols_num-digs_in_num;
...
{
    QString retval="0x";
    int digs_in_num=QString::number(num,16).toUpper().length();
    ...
    retval+=QString::number(num,16).toUpper();
...
QString add_zeros_in_mantissa(QString str_num,quint64 symbols_num_after_point)
{
    ...
    if(str_list.count()>1)
    {
        if(str_list[1].length()<symbols_num_after_point)
            for(int i=0;i<(str_list[1].length()-symbols_num_after_point);i++)
                str_list[1]+="0";

        for(int i=0;i<str_list.count();i++)
        {
            str_retval+=str_list[i];
            if((i+1)<str_list.count())str_retval+=".";
        }
    }
    else str_retval=str_num;
...
{
    QString str;
    QStringList str_list=str_ip.split(".");
    assert(str_list.size());
    for(int i=0;i<str_list.size();i++)
    {
        str_list[i]=add_left_zeros(str_list[i].toUInt(),3);
        str+=str_list[i];
        if(i+1<str_list.size())str+=".";
    }
...
QString del_zeros_in_ip(QString str_ip)
{
    ...
    for(int i=0;i<str_list.size();i++)
    {
        if(str_list[i].size()>1)
        {
            if(str_list[i].data()[0]=="0"&&str_list[i].data()[1]=="0")
                str_list[i].remove(0,2);
            else if(str_list[i].data()[0]=="0")
